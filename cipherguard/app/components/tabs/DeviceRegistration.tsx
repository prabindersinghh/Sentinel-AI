...
function buildLog(platform: string, imei: string, bank: string): Array<{ ms: number; text: string; color: string }> {
  ...
  return [
    { ms: 0,   text: 'RASP integrity checks initializing...', color: '#8A95A8' },
    { ms: 45,  text: 'Root detection: PASS (RootBeer scan + /system/bin/su check)', color: '#00D4AA' },
    { ms: 72,  text: 'Frida injection scan (/proc/self/maps): PASS', color: '#00D4AA' },
    { ms: 89,  text: 'Emulator detection (Build.FINGERPRINT + QEMU props): PASS', color: '#00D4AA' },
    { ms: 134, text: ios
    ...
    { ms: 162, text: ios
      ...
      color: '#8A95A8' },
    { ms: 201, text: 'mTLS 1.3 handshake with Bank Salt Service...', color: '#8A95A8' },
    { ms: 234, text: 'server_salt fetched (32-byte CSPRNG, cached in EncryptedSharedPreferences)', color: '#4A9EFF' },
    { ms: 287, text: 'REGISTRATION event → HashRegistrar.sol via bank gateway', color: '#FF9F1C' },
    { ms: 334, text: 'Orderer confirmation: BFT consensus 3/3 nodes', color: '#4A9EFF' },
    { ms: 378, text: ios
        ? 'App attestation validated against Apple roots: PASS'
        : 'Attestation cert validated against Google attestation roots: PASS',
      color: '#00D4AA' },
    { ms: 401, text: 'Baseline hash written to world state (CouchDB)', color: '#8A95A8' },
    { ms: 422, text: `✓ Fabric TX committed — device registered to ${bank} consortium`, color: '#00D4AA' },
  ];
}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        ...
        <div>
          <div className="cg-card" style={{ marginBottom: 16 }}>
            <h3 style={{ fontWeight: 700, color: '#E8EDF5', marginBottom: 18, fontSize: 15 }}>
              Registration Form
            </h3>
          ...
          {/* SDK info card */}
          <div className="cg-card" style={{ fontSize: 13, color: '#8A95A8' }}>
            <div style={{ fontWeight: 700, color: '#4A9EFF', marginBottom: 10, fontSize: 14 }}>
        ...
        <div>
          <div className="cg-card">
            <div style={{
              ...
              <h3 style={{ fontWeight: 700, color: '#E8EDF5', fontSize: 15 }}>
          ...
          {record && (
            <div className="cg-card-teal fade-up" style={{ marginTop: 16 }}>
              <div style={{
                fontWeight: 800,
                color: '#00D4AA',
                fontSize: 15,
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={{ display: 'block', fontSize: 12, color: '#8A95A8', marginBottom: 6, fontWeight: 600 }}>
        {label}
      </label>
      {children}
    </div>
